...
# Filter unscaled original training data for the desired section
section_data_original = df_train_no_outliers[df_train_no_outliers['26"'] == 1]

# Extract the same rows in scaled data if needed
section_data_scaled = df_train_scaled[df_train_scaled['26"'] == 1]  # Only used if needed
...
for i, (gpm, label) in enumerate(zip(gpm_values, gpm_labels)):
    ...
    # Predict probabilities
    prob_grid = best_SVM_model.predict_proba(grid_scaled)[:, 1].reshape(rpm_grid.shape)
    ...
    # Plot actual unscaled data points for the section
    sns.scatterplot(
        x=section_data_original['WOB'],
        y=section_data_original['TRPM'],
        ax=ax,
        color='red',
        edgecolor='black',
        s=20,
...
plt.suptitle('Probability Contour Map of High ROP in 26" Section', fontsize=14, fontweight='bold')
plt.tight_layout()
plt.show()
